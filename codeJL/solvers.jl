...
#######################################
function secantShermanWoodbury_alg(x,y,obj,gama,sp, itNum,prt,inpt, use_ls)# gamma is the fixed stepsize
    val, ngx, ngy =0,0,0
    ...
    while ( normF> tol ) && iter < itNum
        ...
        α = s'*Jr
        a = r/α - Js/ns2
        Ainv = H - (H*Js*Jr'*H)/(ns2 + Jr'*(H*Js))
        H = Ainv - (Ainv*a*s'*Ainv)/(ns2/α + s'*Ainv*a)
